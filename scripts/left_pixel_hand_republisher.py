...
def callback(data, pub):
    ...
    for person in data.persons:
        ...
        if isValid(person.leftHandParts[0]) and isValid(person.leftHandParts[9]):
            ...
            d_xl = abs(person.leftHandParts[9].pixel.x - person.leftHandParts[0].pixel.x) 
            MP_d_xl = d_xl/2

            if (person.leftHandParts[9].pixel.x >= person.leftHandParts[0].pixel.x):
                MP.x = person.leftHandParts[0].pixel.x + MP_d_xl

            else:
                MP.x = person.leftHandParts[9].pixel.x + MP_d_xl

            
            d_yl = abs(person.leftHandParts[9].pixel.y - person.leftHandParts[0].pixel.y) 
            MP_d_yl = d_yl/2

            if (person.leftHandParts[9].pixel.y >= person.leftHandParts[0].pixel.y):
                MP.y = person.leftHandParts[0].pixel.y + MP_d_yl

            else:
                MP.y = person.leftHandParts[9].pixel.y + MP_d_yl
...
def listener():
    rospy.init_node('left_position_republisher')
    pub = rospy.Publisher("left_republisher", Pixel, queue_size=1)
